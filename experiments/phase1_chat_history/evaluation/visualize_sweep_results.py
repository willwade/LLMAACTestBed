...
def load_results(results_dir: Path, pattern: str) -> pd.DataFrame:
    ...
    if not csv_paths:
        raise FileNotFoundError(f"No CSV files found in {results_dir} matching pattern '{pattern}'")
...
def summarize_runs(results_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return results_df.groupby(group_cols)[metrics].mean().reset_index().sort_values(group_cols)


def plot_metric_by_conversation_window(summary_df: pd.DataFrame, output_dir: Path) -> None:
...
def plot_generation_boxplots(results_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    sns.boxplot(data=melted, x="generation_method", y="score", hue="metric", showfliers=False)
...
def plot_heatmaps(results_df: pd.DataFrame, output_dir: Path, metric: str) -> None:
    ...
    for (context_filter, conv_window), group in heatmap_data.groupby(
        ...
        plt.title(f"{metric.replace('_', ' ').title()} — cf={context_filter}, cw={conv_window}")
        plt.tight_layout()
