...
class Phase1ChatEvaluator:
    ...
    def __init__(self, config_path: str | None = None):
        ...
        """
        self.config_path = config_path or str(Path(__file__).parent.parent / "configs" / "config.yaml")
    ...
    def create_default_methods(self) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
        ...
        # Generation methods
        generation_methods = {
            "lexical": lambda partial, context: evaluator.lexical_generate(partial, context, top_k=3),
            "tfidf": lambda partial, context: evaluator.tfidf_generate(partial, context, top_k=3),
            "embedding": lambda partial, context: evaluator.embedding_generate(partial, context, top_k=3),
            "context_only": lambda partial, context: evaluator.context_only_generate(partial, context),
        }

        # Evaluation metrics
        evaluation_metrics = {
            "embedding_similarity": lambda pred, target: evaluator.embedding_similarity(pred, target),
    ...
    def run_evaluation(self, chat_data_path: str | None = None, sample_size: int | None = None) -> Any:
        ...
        sample_size = sample_size or self.config["evaluation"]["sample_size"]
