...
def main():
    ...
    print(f"Context filter: {args.context_filter} (time_window_hours={args.time_window_hours}, geo_radius_km={args.geo_radius_km})")
...
def analyze_results(results_df, output_path):
    ...
    sns.barplot(
        data=partial_perf, x="partial_method", y="score", hue="metric", ax=axes[0, 0]
    )
    ...
    sns.barplot(
        data=gen_perf, x="generation_method", y="score", hue="metric", ax=axes[0, 1]
    )
